 ...
 * @property Base $base;
 * @property ImportExport importExport;
 */
class BuddyPlugin extends Plugin {
	...
	protected function _setComponents() {
		$this->setComponents( [
			...
			'base'             => Base::class,
			'importExport'    => ImportExport::class,
		] );
	}
	...
	protected function _setRoutes() {
		...
		Event::on(
			...
			function ( RegisterUrlRulesEvent $event ) {
				...
				$event->rules['convergine-contentbuddy/import-export']    = 'convergine-contentbuddy/import-export/index';
			}
		);
	}
		...
		Event::on(
			...
			function ( Event $event ) {
				...
				if ( Craft::$app->getRequest()->getIsCpRequest() ) {
					...
					if (
						( $settings->textAi === 'openai' && $settings->apiToken === '' )
						|| ( $settings->textAi === 'xai' && $settings->xAiApiKey === '' )
						|| ( $settings->textAi === 'anthropic' && $settings->anthropicApiKey === '' )
						|| ( $settings->textAi === 'openrouter' && $settings->openrouterApiKey === '' )
					) {
		...
		if ( Craft::$app->getRequest()->getIsCpRequest() ) {
			...
			Event::on(
				...
				function ( TemplateEvent $event ) {
					...
					if ( $settings->titleFieldEnabled() && $settings->isAvailableForFields() ) {
	...
	public function getCpNavItem(): ?array {
		...
		if ( Craft::$app->getUser()->getIsAdmin() ) {
			...
			$nav['subnav']['import-export']     = [
				'label' => Craft::t( 'convergine-contentbuddy', 'Import/Export' ),
				'url'   => 'convergine-contentbuddy/import-export',
			];
		}
