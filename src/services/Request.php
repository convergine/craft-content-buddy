...
use convergine\contentbuddy\api\text\AnthropicAi;
...
use convergine\contentbuddy\api\text\OpenAiResponses;
...
class Request {
	...
	public function send( $prompt, $maxTokens, $temperature, $isTranslate = false, $instructions = '', $lang = '', $source_lang = '' ) {
		...
		// Must mirror the model each TextApi actually sends, otherwise translation is
		// routed by the content-generation model and vice versa.
		$preferredModel = $isTranslate ? $this->settings->preferredTranslationModel : $this->settings->preferredModel;
		...
		} elseif ( $textAi == 'anthropic' ) {
			$textApi = new AnthropicAi();
		}elseif ( $textAi == 'openrouter' ) {

			$textApi = new OpenRouter();
		} else { //default to openai
			if ( str_contains( $preferredModel, 'gpt-5' ) ) {
				$textApi = new OpenAiResponses();
			} elseif ( $this->settings->openAiAssistantId && $this->settings->translationAi=='openai' && $isTranslate) {
				$textApi = new OpenAiAssistant();
			} else {
				$textApi = new OpenAi();
			}
		...
		Craft::info( 'Using: '.$textAi, 'content-buddy' );
		if ( is_array( $prompt ) ) {
